...
const dreamImages = [
  { title: "Celestial Garden", gradient: "from-purple-500/40 to-pink-500/40", h: "h-64" },
  { title: "Moonlit Waters", gradient: "from-blue-500/40 to-cyan-400/40", h: "h-80" },
  { title: "Aurora Dreams", gradient: "from-green-400/40 to-purple-500/40", h: "h-72" },
  { title: "Starfall Meadow", gradient: "from-pink-500/40 to-yellow-400/40", h: "h-64" },
  { title: "Crystal Twilight", gradient: "from-indigo-500/40 to-rose-400/40", h: "h-80" },
  { title: "Enchanted Mist", gradient: "from-cyan-400/40 to-violet-500/40", h: "h-72" },
];
  ...
  return (
    <section id="gallery" className="relative py-24 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Original abstract dream images */}
        <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
          {dreamImages.map((img, i) => (
            <motion.div
              key={img.title}
              initial={{ opacity: 0, scale: 0.95 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.1, duration: 0.4 }}
              className={`relative group rounded-2xl overflow-hidden ${img.h} break-inside-avoid`}
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${img.gradient}`} />
              <div className="absolute inset-0 opacity-20" style={{
                backgroundImage: `radial-gradient(circle at ${30 + i * 10}% ${40 + i * 5}%, rgba(255,255,255,0.3) 0%, transparent 50%)`,
              }} />
              <div className="absolute inset-0 bg-background/60 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-3">
                <p className="font-serif text-lg text-foreground font-semibold">{img.title}</p>
                <button className="glass-button text-xs flex items-center gap-2 px-4 py-2">
                  <Share2 className="h-3 w-3" /> Share this dream
                </button>
              </div>
